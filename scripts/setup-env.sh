...
collect_configuration() {
    ...
    CONTAINER_NAME=$(prompt_config "Container name" "${ENV_DEFAULT_CONTAINER_NAME:-ttyd-terminal}" "")
    ...
    if prompt_yes_no "Enable GitHub Container Registry (ghcr.io)?"; then
        ...
        GHCR_REPOSITORY=$(prompt_config "  Repository name" "${ENV_DEFAULT_GHCR_REPOSITORY:-claude-terminal}" "")
    else
        USE_GHCR="false"
        GHCR_USERNAME=""
        GHCR_TOKEN=""
        GHCR_REPOSITORY="${ENV_DEFAULT_GHCR_REPOSITORY:-claude-terminal}"
    fi
    
    # Claude Code configuration (optional)
    echo ""
    print_header "Claude Code Configuration (Optional)"
